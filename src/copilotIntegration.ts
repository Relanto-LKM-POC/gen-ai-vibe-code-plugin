...
export class CopilotIntegration {
    ...
    /**
     * Send prompts with file/folder context to Copilot Chat with auto-paste
     */
    public async sendPromptsWithContextToCopilotChatWithAutoPaste(
        message: string, 
        prompts: Prompt[], 
        uri: vscode.Uri, 
        contextType: string
        ...
        try {
            ...
            const contextMessage = `Please help me with this ${contextType}: "${relativePath}" using these prompts: ${prompts.map(p => p.name).join(', ')}`;
            
            let cleanMessage = this.buildPromptOnlyCopilotMessage(contextMessage, prompts);
            
            // Add file/folder context to the message
            cleanMessage += `\n\n**Selected ${contextType.charAt(0).toUpperCase() + contextType.slice(1)}:** \`${relativePath}\`\n`;
            cleanMessage += `Please provide guidance for this ${contextType} using the above prompts.`;
            
            await this.sendToCopilotChatWithAutoPaste(cleanMessage);
            
        } catch (error) {
            vscode.window.showErrorMessage(`Failed to send prompts with context to Copilot: ${error}`);
            this.logToOutput(`Prompt Copilot context auto-paste error: ${error}`);
        }
    }
    ...
    /**
     * Apply instructions with file/folder context to workspace and send to Copilot Chat with auto-paste
     */
    public async applyInstructionsWithContextToWorkspaceWithAutoPaste(
        ...
        try {
            ...
            const instructionSummary = instructions.map(i => i.name).join(', ');
            const relativePath = vscode.workspace.asRelativePath(uri);
            let message = `Please help me analyze this ${contextType}: "${relativePath}" with these instructions: ${instructionSummary}`;
            if (prompt) {
                message += ` using the prompt: ${prompt.name}`;
            }
            
            // Build the message and include file/folder reference
            let cleanMessage = this.buildCompactCopilotMessage(message, instructions, prompt);
            
            // Add file/folder context to the message
            cleanMessage += `\n\n**Selected ${contextType.charAt(0).toUpperCase() + contextType.slice(1)}:** \`${relativePath}\`\n`;
            cleanMessage += `Please analyze and provide guidance for this ${contextType} using the above instructions.`;
            
            await this.sendToCopilotChatWithAutoPaste(cleanMessage);
            
            this.logToOutput(`Applied ${instructions.length} instructions with ${contextType} context: ${relativePath}`);
            if (prompt) {
                this.logToOutput(`Applied prompt: ${prompt.name}`);
            }
        } catch (error) {
            vscode.window.showErrorMessage(`❌ Failed to apply instructions with context: ${error}`);
            this.logToOutput(`Error: ${error}`);
        }
    }
